...
function ContentPlanModal({ onClose }: { onClose: () => void }) {
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[12px] text-[#5a8078] leading-[1.55]">
                            Drive organic traffic and authority with a content strategy tailored to your niche.
                        </p>
                    </div>
                    {/* close */}
                    <button
...
export default function ContentPlanButton() {
    ...
    return (
        <>
            {/*
        Visually identical to the original <Link> — same className, same arrow SVG.
        type="button"        → never submits a parent form
        e.preventDefault()   → stops any inherited anchor behaviour
        e.stopPropagation()  → stops bubbling to parent Link / anchor
      */}
            <button
                type="button"
                onClick={e => { e.preventDefault(); e.stopPropagation(); openModal(); }}
                className="group inline-flex items-center gap-2 rounded-full border-[1.5px] border-[#3cba99] bg-transparent px-3 md:px-6 py-4 text-sm font-medium text-[#0a5c45] dark:text-gray-200 dark:border-[#5EEAD4]/50 transition-all duration-200 hover:bg-[var(--primary-green)] hover:text-white hover:-translate-y-px active:scale-[0.97] sm:px-5 sm:py-3 sm:text-md cursor-pointer"
            >
                Get Content Plan
                <svg
            ...
            {/* portal modal — renders outside all parent DOM */}
            {mounted && open && <ContentPlanModal onClose={closeModal} />}
        </>
    );
}